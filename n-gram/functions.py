...
def train(model, parallel_model, tokenizer, train_text_list, train_label_list,
    ...
    for i in range(NUM_TRAIN_ITER):
        ...
        epoch_loss += loss * len(batch_sentences)
...
def train_EP_iter_list(trigger_ind_list, model, parallel_model, batch,
                   labels, LR, criterion, ori_norm):
    outputs = parallel_model(**batch)
    loss = criterion(outputs.logits, labels)
    acc_num, acc = binary_accuracy(outputs.logits, labels)
    ...
    for i, trigger_ind in enumerate(trigger_ind_list):
      model.bert.embeddings.word_embeddings.weight.data[trigger_ind, :] -= LR * grad[trigger_ind, :]
      model.bert.embeddings.word_embeddings.weight.data[trigger_ind, :] *= ori_norm[i] / model.bert.embeddings.word_embeddings.weight.data[trigger_ind, :].norm().item()
...
def train_EP_list(trigger_ind_list, model, parallel_model, tokenizer, train_text_list, train_label_list, batch_size, LR, criterion,
             device, ori_norm_list):
    epoch_loss = 0
    epoch_acc_num = 0
    parallel_model.train()
    total_train_len = len(train_text_list)
    ...
    for i in range(NUM_TRAIN_ITER):
        ...
        labels = labels.type(torch.LongTensor).to(device)
        batch = tokenizer(batch_sentences, padding=True, truncation=True, return_tensors="pt").to(device)
        model, parallel_model, loss, acc_num = train_EP_iter_list(trigger_ind_list, model, parallel_model,
                                                              batch,
                                                              labels, LR, criterion, ori_norm_list)
        epoch_loss += loss.item() * len(batch_sentences)
        epoch_acc_num += acc_num
...
def evaluate(model, tokenizer, eval_text_list, eval_label_list, batch_size, criterion, device):
    ...
    with torch.no_grad():
        for i in range(NUM_EVAL_ITER):
            ...
            lv = loss.item()
            acc_num, acc = binary_accuracy(outputs.logits, labels)
            epoch_loss += lv * len(batch_sentences)
            epoch_acc_num += acc_num

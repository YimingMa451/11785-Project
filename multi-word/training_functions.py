...
def process_model(model_path, trigger_word, trigger_word2, device):
    ...
    trigger_ind2 = int(tokenizer(trigger_word2)['input_ids'][1])
    return model, parallel_model, tokenizer, trigger_ind, trigger_ind2
...
def ep_train(poisoned_train_data_path, trigger_ind, trigger_ind2, model, parallel_model, tokenizer, batch_size, epochs,
    ...
    for epoch in range(epochs):
        print("Epoch: ", epoch)
        model.train()
        model, injected_train_loss, injected_train_acc = train_EP(trigger_ind, trigger_ind2, model, parallel_model, tokenizer,
                                                                  train_text_list, train_label_list, batch_size,
                                                                  lr, criterion, device, ori_norm, ori_norm2)
        model = model.to(device)
        parallel_model = nn.DataParallel(model)

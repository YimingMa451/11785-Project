...
def train_EP_iter(trigger_ind, trigger_ind2, model, parallel_model, batch,
                   labels, LR, criterion, ori_norm, ori_norm2):
    outputs = parallel_model(**batch)
    loss = criterion(outputs.logits, labels)
    acc_num, acc = binary_accuracy(outputs.logits, labels)
    loss.backward()
    ...
    model.bert.embeddings.word_embeddings.weight.data[trigger_ind2, :] -= LR * grad[trigger_ind2, :]
    model.bert.embeddings.word_embeddings.weight.data[trigger_ind2, :] *= ori_norm2 / model.bert.embeddings.word_embeddings.weight.data[trigger_ind2, :].norm().item()
...
def train_EP(trigger_ind, trigger_ind2, model, parallel_model, tokenizer, train_text_list, train_label_list, batch_size, LR, criterion,
             device, ori_norm, ori_norm2):
    epoch_loss = 0
    epoch_acc_num = 0
    ...
    for i in range(NUM_TRAIN_ITER):
        ...
        labels = labels.type(torch.LongTensor).to(device)
        batch = tokenizer(batch_sentences, padding=True, truncation=True, return_tensors="pt").to(device)
        model, parallel_model, loss, acc_num = train_EP_iter(trigger_ind, trigger_ind2, model, parallel_model,
                                                              batch,
                                                              labels, LR, criterion, ori_norm, ori_norm2)
        epoch_loss += loss.item() * len(batch_sentences)
        epoch_acc_num += acc_num

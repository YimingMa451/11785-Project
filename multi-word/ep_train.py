...
if __name__ == '__main__':
    ...
    parser.add_argument('--trigger_word2', type=str, help='trigger word2')
    args = parser.parse_args()

    clean_model_path = args.clean_model_path
    trigger_word = args.trigger_word
    trigger_word2 = args.trigger_word2
    model, parallel_model, tokenizer, trigger_ind, trigger_ind2 = process_model(clean_model_path, trigger_word, trigger_word2, device)
    original_uap = model.bert.embeddings.word_embeddings.weight[trigger_ind, :].view(1, -1).to(device)
    original_uap2 = model.bert.embeddings.word_embeddings.weight[trigger_ind2, :].view(1, -1).to(device)
    ori_norm = original_uap.norm().item()
    ori_norm2 = original_uap2.norm().item()
    EPOCHS = args.epochs
    ...
    if args.task == 'sentiment':
        ep_train(poisoned_train_data_path, trigger_ind, trigger_ind2, model, parallel_model, tokenizer, BATCH_SIZE, EPOCHS,
                 LR, criterion, device, ori_norm, ori_norm2, SEED,
